...
namespace Lasm.Bolt.UniversalSaver
{
    ...
    public sealed class SaveUniversalVariables : UniversalSaveUnit
    {
        ...
        {
            return (usePersistantDataPath) ? Application.persistentDataPath + "/Savegame/" + flow.GetValue<string>(fileName) : flow.GetValue<string>(path) + "/" + flow.GetValue<string>(fileName);
        }
