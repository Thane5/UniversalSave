...
namespace Lasm.Bolt.UniversalSaver
{
    ...
    public sealed class UniversalSaveExists : UniversalSaveUnit
    {
        ...
        {
            if (File.Exists((usePersistantDataPath) ? Application.persistentDataPath + "/Savegame/" + flow.GetValue<string>(fileName) : flow.GetValue<string>(path) + "/" + flow.GetValue<string>(fileName)))
            {
                return @true;
            }
